...
export const Laboratory: React.FC = () => {
  const items = [
    { id: 1, patient: 'Samantha Ratnayake', test: 'Full Blood Count (FBC)', lab: 'Lanka Labs Colombo', date: '2026-07-20', status: 'COMPLETED' },
    { id: 2, patient: 'Rohan Fernando', test: 'Lipid Profile', lab: 'Asiri Laboratories', date: '2026-07-15', status: 'COMPLETED' },
    { id: 3, patient: 'Dilshan Jayawardene', test: 'HbA1c Blood Glucose', lab: 'Lanka Labs Kurunegala', date: '2026-07-27', status: 'PENDING' },
  ];
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-400">Order diagnostics, track blood panel statuses, and upload clinical findings.</p>
        </div>
          ...
          <table className="w-full text-left text-sm text-slate-300">
            <thead className="bg-slate-950 text-slate-400 text-xs font-semibold uppercase border border-slate-850">
              <tr>
                ...
                <th className="px-4 py-3">Diagnostic Test Requested</th>
            ...
            <tbody className="divide-y divide-slate-850">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-850/30">
                  ...
                  <td className="px-4 py-4">{item.lab}</td>
                  ...
                  <td className="px-4 py-4">
                    <span className={`inline-flex items-center space-x-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                      item.status === 'COMPLETED' ? 'bg-clinical-500/10 text-clinical-400' : 'bg-yellow-500/10 text-yellow-400'
                    }`}>
                  ...
                  <td className="px-4 py-4 text-right">
                    <button className="inline-flex items-center space-x-1 text-xs text-clinical-400 hover:underline font-semibold">
